...
func transition_to_gamecam():
	if intro_cam_finished and intro_dialog_finished:
		$Player/ThirdPersonCamera.get_camera().visible = true
		$Player/ThirdPersonCamera.get_camera().set_current(true)
...
func _on_ui_intro_finished():
	...
	var game_cam_position = $Player/ThirdPersonCamera.get_camera().get_global_transform().origin
	var game_cam_rotation = $Player/ThirdPersonCamera.get_camera().get_global_transform().basis.get_rotation_quaternion()
